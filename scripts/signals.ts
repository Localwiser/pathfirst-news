 ...
 *
 * Two rules hold for every source, not just the ones wired up today
 * (signals.json _meta.crawl_rules — Google Play, GitHub issues and any
 * changelog page will hit the same thing):
 *
 *   1. An empty read is 未取到数据, never 无此问题. A throttled source answers
 *      200 with nothing, which reads as "nobody complains about this product"
 *      — a positive conclusion we would have invented. Everything goes through
 *      nonEmpty(): three tries, growing pauses, then a log line and no signal.
 *   2. Under three reports, say nothing. One anonymous 1-star review on a card
 *      reads as a widespread problem. Same floor as U1's user reports.
 *
...
const UA =
  "PathfirstSignalsBot/1.0 (+https://github.com/Localwiser/pathfirst-news)";
...
/**
 * Apple throttles a burst of feed pages; wait between them, and give page 1 a few
 * tries. Being impolite here does not fail loudly — it comes back as 200 with no
 * reviews, i.e. as fake good news — so the pauses are deliberately generous.
 */
const PAGE_PAUSE_MS = 2_000;
const FEED_ATTEMPTS = 3;
/** Between competitors, so a run of a dozen apps does not read as a burst. */
const COMPETITOR_PAUSE_MS = 3_000;
/**
...
export function robotsAllows(
  ...
  const rules = (
    mine.length ? mine : groups.filter((g) => g.agents.includes("*"))
  ).flatMap((g) => g.rules);
  ...
  for (const r of rules)
    if (
      matches(r.path) &&
      (!best ||
        r.path.length > best.len ||
        (r.path.length === best.len && r.allow))
    )
...
/**
 * Empty is never a finding (signals.json _meta.crawl_rules.empty_is_not_zero).
 *
 * A source that answers 200 with nothing looks exactly like a product nobody
 * complains about — and that reads as a positive conclusion, which is the one
 * thing a crawl must never invent. 200 + empty is more dangerous than a 404:
 * the 404 throws and stops, the empty array gets written to the signal库.
 *
 * So every source goes through here: try up to FEED_ATTEMPTS times with growing
 * pauses, and return null when all of them came back empty. The caller then
 * logs 未取到数据 and produces no signal — never "no one complained".
 *
 * `size` says what counts as empty for that source, so a page that rendered
 * nothing (a few characters of chrome) is as empty as a missing one.
 */
export async function nonEmpty<T>(
  fetchOnce: () => Promise<T>,
  size: (value: T) => number,
  ...
  for (let attempt = 0; attempt < FEED_ATTEMPTS; attempt++) {
    if (attempt) await pause(pauseMs * (attempt + 1));
    last = await fetchOnce();
    if (size(last) > 0) return last;
  }
  return null;
}

/** What a Zendesk section gave us: the in-window text, and how many articles exist at all. */
type Section = { text: string; articles: number };
 ...
 *
 * `articles` counts everything the API returned, in the window or not: a
 * section with 16 entries and none in the last year is a fact about the
 * product, while a section that returned nothing at all is a failed read.
 * The two must not end up in the log as the same sentence.
 */
async function zendeskSection(
  sectionUrl: string,
  since: string,
): Promise<Section> {
  ...
  for (let page = 0; next && page < 5; page++) {
    ...
    for (const a of body.articles)
      if (a.created_at.slice(0, 10) >= since)
        parts.push(
          `${a.created_at.slice(0, 10)} ${a.title}\n${pageText(a.body ?? "")}`,
        );
    next = body.next_page;
  }
  return { text: parts.join("\n\n").slice(0, PAGE_CHARS), articles };
}
 ...
 * next for that reason. That is what nonEmpty() is for; the caller logs
 * 未取到数据 when it comes back empty, never "no one complained".
 */
async function appReviews(id: number, country: string): Promise<Review[]> {
  ...
  const first = await nonEmpty(
    async () => (await reviewPage(page(1))) ?? [],
    (r) => r.length,
  );
  if (!first) return [];
  const out = [...first];
...
const Category = z.object({
  ...
  label_en: z
    .string()
    .describe("English, at most 8 words, a paraphrase, never a quote"),
...
const ChangelogResult = z.object({
  entries_in_window: z
    .number()
    .int()
    .describe("dated release entries inside the window"),
  latest_date: z
    .string()
    .describe("YYYY-MM-DD of the newest entry in the window, or empty"),
...
function changelogRecord(
  ...
  return {
    ...
    confidence:
      r.entries_in_window >= 6
        ? "high"
        : r.entries_in_window >= 3
          ? "medium"
          : "low",
...
async function run(dry: boolean) {
  ...
  for (const c of config.combinations)
    for (const comp of c.competitors) {
      const skip = (source: string, reason: string) => {
        ...
        console.log(
          `  · skip ${c.industry}-${c.market} ${comp.name} ${source}: ${reason}`,
        );
      };
      ...
      if (!comp.changelog) skip("changelog", "no public changelog");
        ...
        skip(
          "changelog",
          `terms ${comp.changelog.terms.verdict}: ${comp.changelog.terms.note ?? ""}`,
        );
      else if (!(await robotsOk(comp.changelog.url)))
        skip("changelog", "robots.txt disallows");
      else
        try {
          // Two different nothings, and they must never share a log line: a page
          // that would not give us anything (未取到数据), and a changelog we read
          // in full that has no entry in the window (a fact about the product).
          const zendesk = comp.changelog.kind === "zendesk";
          const section = await nonEmpty<Section>(
            async () =>
              zendesk
                ? await zendeskSection(comp.changelog!.url, since)
                : {
                    text: pageText(await get(comp.changelog!.url)),
                    articles: 1,
                  },
            // For a plain page the text IS the read: too short means it gave us
            // nothing. For Zendesk the API's article count says whether we read it.
            (s) =>
              zendesk
                ? s.articles
                : s.text.length >= MIN_CHANGELOG_CHARS
                  ? s.text.length
                  : 0,
          );
          if (!section)
            skip(
              "changelog",
              `未取到数据：${FEED_ATTEMPTS} 次都返回空，不作结论`,
            );
          else if (section.text.length < MIN_CHANGELOG_CHARS)
            skip(
              "changelog",
              `读到 ${section.articles} 条，${since} 之后一条都没有`,
            );
          else {
            const text = section.text;
            console.log(
              `  ✓ ${c.industry}-${c.market} ${comp.name} changelog: ${text.length} chars`,
            );
            if (!dry) {
      ...
      try {
        ...
        if (!all.length)
          skip(
            "app_store",
            `未取到数据：评价 feed ${FEED_ATTEMPTS} 次都返回 0 条，不作结论`,
          );
        else if (low.length < MIN_LOW_REVIEWS)
          skip(
            "app_store",
            `1–2 星评价只有 ${low.length} 条，少于 ${MIN_LOW_REVIEWS}，不作结论`,
          );
        else if (!dry) {
      ...
      // Slow down between competitors: the throttle's answer is silence, not an error.
      await pause(COMPETITOR_PAUSE_MS);
    }
  ...
  for (const [key, records] of Object.entries(out)) {
    ...
    const file = {
      _meta: { generated: today, window_start: since, model: MODEL },
      signals: records,
    };
  ...
  writeFileSync(
    join(OUT, "log/latest.json"),
    `${JSON.stringify({ date: today, skipped }, null, 2)}\n`,
  );
  writeFileSync(
    join(OUT, "index.json"),
...
export function fileProblems(file: { signals?: SignalRecord[] }): string[] {
  ...
  for (const [i, r] of (file.signals ?? []).entries()) {
    ...
    for (const k of Object.keys(r))
      if (!allowed.has(k)) errs.push(`${at}: field "${k}" is not allowed`);
    if (!["C3a", "C3b"].includes(r.signal_id))
      errs.push(`${at}: signal_id ${r.signal_id}`);
    if ([...r.value].length > 80)
      errs.push(`${at}: value longer than 80 characters`);
    if (r.value_en.length > 200)
      errs.push(`${at}: value_en longer than 200 characters`);
    for (const d of r.detail)
      if ([...d].length > 24) errs.push(`${at}: detail "${d}" too long`);
    for (const d of r.detail_en)
      if (d.length > 72) errs.push(`${at}: detail_en too long`);
    if (
      !r.source_urls.length ||
      r.source_urls.some((u) => !/^https:\/\//.test(u))
    )
...
function check() {
  ...
  for (const c of config.combinations)
    for (const comp of c.competitors)
      if (
        comp.changelog &&
        !["allowed", "forbidden", "unclear"].includes(
          comp.changelog.terms.verdict,
        )
      )
